...
class RTR {
  constructor(platform, code) {
    ...
    Object.assign(this.functions, {
      ...
      event: (name) => this.runEvent(name),
    });

    this.vars = [{
      platform: Object.assign({ name: "unknown", version: "unknown" }, platform),
      rtr: { version: "1.5", environment: { type: "javascript", } },
  ...
  parseData(line) {
    ...
    if ((line.startsWith("[") && line.endsWith("]")) || (line.startsWith("{") && line.endsWith("}"))) {
      ...
      if (func.match(/^[a-zA-Z0-9_]+$/) !== null && !func.includes(".")) {
        return { cmd: func, type: "func",
    ...
    for (; i < lines.length; i++) {
      ...
      if (regex.test(line)) {
        while (regex.test(line)) {
          ...
          let line_data = line.slice(line.indexOf("(") + 1, line.lastIndexOf(")") - 1)
  ...
  setVar(variable, val, assignment) {
    let scope = this.vars[this.vars.length - 1];
  ...
  evaluateNode(node) {
    ...
    switch (node?.type) {
      ...
      case "func": {
        ...
        if (cmd.endsWith("=") && typeof node.variable !== "undefined") return this.setVar(node.variable, this.evaluateNode(data[0]), cmd);
        switch (cmd) {
          ...
          default: {
            try {
              if (this.functions[cmd]) return this.functions[cmd](...data.map(d => this.evaluateNode(d)));

              const func = this.getVar(cmd);
  ...
  call(code) {
    for (let i = 0; i < code.length; i++) {
      ...
      if (this.getVar(" returned"))
        return this.evaluateNode(this.getVar(" return_val"));
    }
  }
}
